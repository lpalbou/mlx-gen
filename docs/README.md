...
- [Masked editing](masked-editing.md): the canonical masked-edit page — request contract, per-model route matrix with proof grades, per-family behavior (Qwen edit and base routes, Z-Image turbo/non-turbo, FLUX.2 Klein, video masks), and route selection advice.
