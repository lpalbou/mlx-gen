...
| `Qwen/Qwen-Image`, `AbstractFramework/qwen-image-4bit`, `AbstractFramework/qwen-image-2512-8bit` | `qwen.base-inpaint` (native, no sidecar) | Visual smoke on 4bit and 2512-8bit ([proof bundle](assets/validation/masked-edit-2026-07-15/README.md)); source/bf16 rows share the wiring without a dedicated run |
| `AbstractFramework/z-image-turbo-8bit` | `z-image.inpaint` | Validated visual QA ([edit capabilities](edit-capabilities.md)) |
| `Tongyi-MAI/Z-Image`, `AbstractFramework/z-image-8bit` (non-turbo) | `z-image.inpaint` | Visual smoke on the q8 row ([proof bundle](assets/validation/masked-edit-2026-07-15/README.md)) |
| FLUX.2 Klein distilled and base rows | `flux2.inpaint` | Model-backed smoke (distilled 4-step, base CFG, reference-conditioned case) |
...
`effective_steps` and `masked_warm_start_strength`. This differs from the other masked routes
...
### Z-Image (`z-image.inpaint`, turbo and non-turbo)

Native per-step latent compositing on both rows. Turbo runs 9 steps without guidance. The
non-turbo row defaults to guidance-free generation; pass an explicit `--guidance` (the proof run
used `--guidance 4` at 30 steps) for prompt-adherent masked edits. Mask resampling: NEAREST at
the latent grid, 50% threshold.
...
- Local edits without any sidecar or edit checkpoint: base Qwen native route or Z-Image.
...
- [Masked edit expansion bundle (2026-07-15)](assets/validation/masked-edit-2026-07-15/README.md):
  base-Qwen native (4bit, 2512-8bit) and Z-Image non-turbo rows, one shared object-insertion
  case with outside-mask preservation measurements and the runtime-truth metadata sidecar.
