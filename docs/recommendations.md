...
| `128+ GB` | Everything above. | Everything above, plus room for larger or more exploratory runs. | Add native `AbstractFramework/wan2.2-ti2v-5b-diffusers-8bit` high-resolution video profiles such as `1280x704, 17 frames, 20 steps`, and `minimax-h3-turbo-544p` for video with a synchronized stereo soundtrack (`--quantize 8`, 80 GB MLX peak at `960x544`, see [MiniMax-H3](minimax-h3.md)). | Add published `seedvr2-7b` five-second `29/8` video restore and larger Wan experimentation with real headroom. |
